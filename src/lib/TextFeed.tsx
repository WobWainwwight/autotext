import {ChangeEvent, FC, KeyboardEvent, MouseEvent, MouseEventHandler, useEffect, useRef, useState} from "react";
import {addCardInPosition, appendCardToDoc, Card, D, deleteAtPosition, fetchLocalDoc, setTextOnCard} from "./automerge";

type TextFeedProps = {
    actorName: string
}

export const TextFeed: FC<TextFeedProps> = ({actorName}) => {
    ...
    return <div id={"text-space"} className={"flex flex-col justify-center min-w-[40%] min-h-screen p-y-5"} onClick={handleClick}>
        { !state.cards.length
